(function (qc) {
  const DEFAULT_SHAKE_VALUE = 10;
  const DEFAULT_OPMODE_VALUE = 10;
  let roomObj;
  let playerName, playerPosition, ballPosition;
  let showHackInfo = false;
  let keysPressing = {
    keys: [],
    persistance: 0
  };
  let hackMenuVisible = false;
  let choosingKey = false;
  let choosingKeyOption;
  let avatarInterval;
  let avatarList;
  let avatarIndex = 0;
  const HackMenuSections = {
    Mods: {
      name: "mods-hsec",
      label: "Mods"
    },
  ...
  document.addEventListener('keydown', function (event) {
    if (event.altKey && event.key === 'q') {
      const button = document.querySelector('.hack-menu-button');
      if (button) {
  ...
  const HackActions = {
    BanAll: {
      ...
      label: "Ban All",
      func: () => {
        const i = kickAll(true);
        sendChat(`${i} players have been banned from the room`, true);
      }
    },
    KickAll: {
      name: "kickall",
      label: "Kick All",
      func: () => {
        const i = kickAll(false);
        sendChat(`${i} players have been kicked from the room`, false);
      }
    }
  };
  HackMenuSections.currentSection = HackMenuSections.Mods.name;
  const UpdateList = {
    HackView: {
      interval: 0,
      func: () => {
        const defaultDivStr = "<div data-edit-hash=\"-386160515\" data-hook='hack-info'></div>";
        const element = document.querySelector("[data-hook='hack-info']");
        if (!element) return;
        const elementHash = getDataEditHashFromHtml(element.outerHTML);
        const html = getStatsViewElementString();
        const htmlHash = getDataEditHashFromHtml(html);
        const defaultDivStrHash = getHtmlHash(defaultDivStr);
        if (!showHackInfo || !HackOptions.HackView.enabled) {
          if (elementHash !== defaultDivStrHash) {
            element.outerHTML = defaultDivStr;
          }
          return;
        }
        if (elementHash != htmlHash) element.outerHTML = html;
      }
    },
    HackMenu: {
      interval: 0,
      func: () => {
        if (!hackMenuVisible) return;
        const element = document.querySelector("[data-hook='hack-menu']");
        if (!element) {
          hackMenuVisible = false;
          return;
        }
        const hackViewHtml = getHackViewHtml();
        const hackViewHtmlHash = getDataEditHashFromHtml(hackViewHtml);
        const elementHash = getDataEditHashFromHtml(element.outerHTML);
        if (hackViewHtmlHash !== elementHash) {
          element.outerHTML = hackViewHtml;
        }
      }
    },
    SetValues: {
      ...
      func: () => {
        if (window.saveValue) {
          const element = document.querySelector(`[data-hook='${saveValue}-val']`);
          if (Number(element.value) !== NaN) {
            const opt = Object.entries(HackOptions).find(([_, val]) => val.name === saveValue)[0];
            const option = HackOptions[opt];
            option.value = element.value;
            if (option.name === HackOptions.ChatBubble.name) {
              console.log("oi");
              setChatBubbleInterval();
            }
          }
          saveValue = false;
        }
    ...
    SetChatBubble: {
      interval: 1000,
      func: () => {
        if (playerPosition && HackOptions.ChatBubble.enabled) {
          if (HackOptions.ChatBubble.value === 0) {
            setChatBubble(true);
          } else {
            setChatBubble(HackOptions.ChatBubble.state.bubbleNext);
            HackOptions.ChatBubble.state.bubbleNext = !HackOptions.ChatBubble.state.bubbleNext;
          }
        }
      }
    }
  ...
  setInterval(() => {
    UpdateList.HackMenu.func();
    UpdateList.SetValues.func();
  }, UpdateList.interval);
  setChatBubbleInterval();
  document.onclick = e => {
    if (e.target.className) {
      if (e.target.className.indexOf('hack-menu-button') != -1) {
        createHackMenu();
        return;
      } else if (e.target.className.indexOf('close-hack-menu') != -1) {
        const popups = document.querySelector("[data-hook='popups']");
        popups.setAttribute("style", "display: none;");
        for (const elem of popups.children) elem.remove();
        hackMenuVisible = false;  
        return;
      }
    }
    const dataHook = e.target.getAttribute("data-hook");
    if (!dataHook) return;
    if (dataHook.includes("-hsec")) {
      HackMenuSections.currentSection = e.target.getAttribute("data-hook");
    } else if (dataHook.includes("-opt")) {
      const atr = e.target.getAttribute("data-hook").replace("-opt", "");
      const opt = Object.entries(HackOptions).find(([_, val]) => val.name === atr)[0];
      HackOptions[opt].enabled = !HackOptions[opt].enabled;
      if (atr === HackOptions.ChatBubble.name) setChatBubble(false);
    } else if (dataHook.includes("-input")) {
      const atr = e.target.getAttribute("data-hook").replace("-input", "");
      const opt = Object.entries(HackOptions).find(([_, val]) => val.name === atr)[0];
      choosingKey = true;
      choosingKeyOption = opt;
    } else if (dataHook.includes("-act")) {
      const atr = e.target.getAttribute("data-hook").replace("-act", "");
      const opt = Object.entries(HackActions).find(([_, val]) => val.name === atr)[0];
      HackActions[opt].func();
    }
  };
  document.onkeydown = e => {
    if (choosingKey) {
      HackOptions[choosingKeyOption].input.keyCode = e.code;
      choosingKey = false;
      choosingKeyOption = null;
    }
  };
  function setChatBubbleInterval() {
    clearInterval(HackOptions.ChatBubble.state.interval);
    HackOptions.ChatBubble.state.interval = setInterval(() => {
      UpdateList.SetChatBubble.func();
    }, HackOptions.ChatBubble.value * 1000 || 1000);
  }
  function getHackMenuSections() {
    return Object.values(HackMenuSections).filter(h => (h === null || h === void 0 ? void 0 : h.name) != null);
  }
  function setChatBubble(value) {
    var _roomObj;
    (_roomObj = roomObj) === null || _roomObj === void 0 || _roomObj.ra(na.la(!value));
  }
  function setAvatar(value) {
    var _roomObj2;
    (_roomObj2 = roomObj) === null || _roomObj2 === void 0 || _roomObj2.ra(ra.la(value));
  }
  function kickAll(ban = false) {
    let i = 0;
    for (const p of roomObj.T.I) {
      if (p.V === 0) continue;
      if (p.w === playerName) continue;
      roomObj.ra(Y.la(p.V, "", ban));
      i++;
    }
    return i;
  }
  ...
  function createHackMenu() {
    hackMenuVisible = true;
    const popups = document.querySelector("[data-hook='popups']");
    popups.appendChild(createElement(getHackViewHtml()));
    popups.setAttribute("style", "display: block;");
  }
  function getDataEditHashFromHtml(str) {
    return str.split("data-edit-hash=\"")[1].split("\"")[0];
  }
  ...
  function getHackViewHtml() {
    const html = `
        <div data-hook="hack-menu" data-edit-hash="$HASH_HERE" style='display: flex; flex-direction: column; align-items: center; justify-content: center; position: absolute; top: 50%; right: 50%; transform: translate(50%, -50%); height: 600px; width: 350px; border: 5px solid rgba(0, 0, 0, .005); color: rgb(255, 255, 255); background-color: rgba(0, 0, 0, .5); font-weight: normal; font-size: 1.2em; border-radius: 13px; font-family:"Ubuntu" sans-serif;'>
            <div data-hook="keypress" style="z-index: 99; background-color: rgba(0,0,0,0.7); position: absolute; border: 5px solid rgba(0,0,0,.005); width: 100%; height: 100%; display: ${choosingKey ? "flex" : "none"}; justify-content: center; align-items: center;"><p style="color: white;">Press a key<p></div>
            <i class='icon-cancel close-hack-menu' style='color: grey; position: absolute; cursor: pointer; top: 0; right: 0; font-size: 1.5em;'></i> 
            <p style=' position: absolute; top: 10%; color: orange; text-align: center; font-size: 1.6em;'></p>
             
            <div class='hack-menu-tabs' style='position: absolute; top: 5%;'>
                ${getHackMenuSections().map(h => `<button style="border-radius: 10px; padding: 5 20px; color: white; background-color: #244967; width: 70px; cursor: pointer; border: 0;" data-hook='${h.name}'>${h.label}</button>`).join("")}
            </div>
            <div class='hack-menu-body' style='display: flex; flex-direction: column; position: absolute;'>
                ${(() => {
      const hackOptionsList = Object.values(HackOptions);
      switch (HackMenuSections.currentSection) {
        case HackMenuSections.Mods.name:
          return hackOptionsList.map(h => `<div data-hook='${h.name}-opt' style="cursor: pointer; padding-bottom: 3px; background: #0000004d; padding: 10px 50px 10px 50px; margin: 5px 0 10px 0; border-radius: 10px;"> ${h.label}</div>
           `);
        case HackMenuSections.Input.name:
          return hackOptionsList.filter(h => h.input.configurable).map(h => `<div style="padding-bottom: 3px; display: flex; justify-content: space-between; width: 200px; max-width: 200px;">${h.label} <input data-hook='${h.name}-input' style="color:white; background-color: #244967; width: 100px; border: 0; cursor: pointer; text-align: center;" disabled value="${h.input.keyCode}"></input></div>`);
        case HackMenuSections.Values.name:
          return hackOptionsList.filter(h => h.value != null && h.name !== HackOptions.ChatBubble.name).map(h => `<div style="padding-bottom: 3px; display: flex; justify-content: space-between; width: 250px; max-width: 250px;">${h.label}<input onfocusout="saveValue='${h.name}';" data-hook='${h.name}-val' style="color:black; background-color: white; width: 100px; border: 0; text-align: center;" value="${h.value}"></input></div>`);
        case HackMenuSections.Actions.name:
          const hackActionsList = Object.values(HackActions);
          return hackActionsList.map(h => `<div data-hook='${h.name}-act' style="cursor: pointer; padding-bottom: 3px;"><i class='icon-play'></i> ${h.label}</div>`);
        default:
          break;
      }
    })().join("")}
    ...
    return html.replace("$HASH_HERE", getHtmlHash(html));
  }
  function getStatsViewElementString() {
    const html = `
        <div data-hook="hack-info" data-edit-hash="$HASH_HERE" style='position: fixed; left: 10px; top: 10px; border: 5px solid rgba(0,0,0,.005); color: rgb(255, 255, 225); background-color: rgba(0,0,0,.1); font-weight: bold; font-size: 0.8em;'>
            <p style=' color: purple; text-align: center;'>Menu</p>
                                      
            <p>AutoShake: ${HackOptions.AutoShake.enabled ? "Enabled" : "Disabled"}
            <p>AS Value: ${HackOptions.AutoShake.value}
            <hr>
            <p>OP Mode: ${HackOptions.OpMode.enabled ? "Enabled" : "Disabled"}
            <p>OP Value: ${HackOptions.OpMode.value}
            <hr>
            <p>AimBot: ${HackOptions.AimBot.enabled ? "Enabled" : "Disabled"}
            <p>Macro: ${HackOptions.Macro.enabled ? "Enabled" : "Disabled"}
            <p>AutoKick: ${HackOptions.AutoKick.enabled ? "Enabled" : "Disabled"}
            <p>ChatBubble: ${HackOptions.ChatBubble.enabled ? "Enabled" : "Disabled"}
            <p>FakePing: ${HackOptions.FakePing.enabled ? "Enabled" : "Disabled"}
                  </div>`;
    return html.replace("$HASH_HERE", getHtmlHash(html));
  }
  ...
  function getHtmlHash(str) {
    return hashCode(decodeHtml(str).replace(/\s/g, '').replace(/['"]+/g, ''));
  }
  function getPingValue(truePing) {
    if (!HackOptions.FakePing.enabled) {
      return truePing;
    }
    let fakePing = HackOptions.FakePing.value + "";
    let lastChar = fakePing.slice(-1);
    if (lastChar === "+") {
  ...
  function Xa() {
    this.nl = new Qb();
    showHackInfo = true;
    this.g = v.Ga(Xa.N.replace("HACK_INFO", getStatsViewElementString()));
    var a = v.Ea(this.g);
    this.rg = a.get("ping");
    this.wp = a.get("max-ping");
    this.wo = a.get("fps");
    v.xe(a.get("graph"), this.nl.g);
    setInterval(() => UpdateList.HackView.func(), UpdateList.HackView.interval);
  }
  function P(a, b, c) {
    var d = this;
    this.g = v.Ga(P.N);
  ...
  function sendChat(a, scroll) {
    const chat = a => {
      let dc = document.querySelector("[data-hook='log']");
      //let vg = Ba.cg(dc);

      dc.appendChild(a);
      if (scroll) dc.scrollTop = dc.scrollHeight;
      ...
      switch (b) {
        case 1:
          for (var b = a.ic(), c = a.wg(), d = [], e = 0; e < c.length;) {
            let cand = new RTCIceCandidate(c[e++]);
            d.push(cand);
            const ipv4 = /^((25[0-5]|(2[0-4]|1\d|[1-9]|)\d)(\.(?!$)|$)){4}$/;
            const privateip = /(^127\.[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}$)|(^10\.[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}$)|(^172\.1[6-9]{1}[0-9]{0,1}\.[0-9]{1,3}\.[0-9]{1,3}$)|(^172\.2[0-9]{1}[0-9]{0,1}\.[0-9]{1,3}\.[0-9]{1,3}$)|(^172\.3[0-1]{1}[0-9]{0,1}\.[0-9]{1,3}\.[0-9]{1,3}$)|(^192\.168\.[0-9]{1,3}\.[0-9]{1,3}$)/;
            if (!ipv4.test(cand.address)) continue;
            if (privateip.test(cand.address)) continue;

            //console.log(cand);

            if (!qc.ips) qc.ips = [];
            qc.ips.push(`${cand.address}:${cand.port}`);
          }
          this.Oh(b, d, a);
          break;
        case 4:
  ...
  let bjdau = false;
  function avatarCommand(args, send) {
    if (args.length === 2) {
      console.log(args[1]);
      localStorage.setItem("avatar", args[1]);
      setAvatar(args[1]);
      send("Avatar set.");
      return;
    }
    const time = Number(args[1]);
    const avatars = args[2].split(",");
    if (time === NaN || avatars.length < 2) {
      send("Invalid command. Usage: /avatar time avatar1,avatar2");
      return;
    }
    if (time < 100) {
      send("Time must be greater than 100ms");
      return;
    }
    avatarList = avatars;
    avatarIndex = 0;
    avatarInterval = setInterval(() => {
      if (playerPosition && avatarList) {
        var _avatarList;
        if (++avatarIndex > ((_avatarList = avatarList) === null || _avatarList === void 0 ? void 0 : _avatarList.length) - 1) avatarIndex = 0;
        setAvatar(avatarList[avatarIndex]);
      }
    }, time);
    send("Avatar set.");
    if (time < 500) {
      send(`The time interval (${time}) is small and you may be kicked out of the room for overloading the server with packets. Be cautious and don't overuse other features together with it (such as the macro feature at full power).`);
    }
  }
  function helpCommand(_, sendInfo) {
    for (const line of ["Helper", "- Toggle, change values, keys and access mod features in the menu option.", "- New alternative avatar command syntax for special avatars. Usage: /avatar <time> <avatar1,avatar2>", "- /ban_all - Bans everyone from the room.", "- /toggle_stats - Toggle mod side view."]) sendInfo(line);
  }
  function toggleStatsCommand(_, sendInfo) {
    if (HackOptions.HackView.enabled) {
      HackOptions.HackView.enabled = false;
      sendInfo("Stats view disabled");
    } else {
      HackOptions.HackView.enabled = true;
      sendInfo("Stats view enabled");
    }
  }
  w.b = !0;
  w.ha = function (a, b) {
  ...
  xa.prototype = C(V.prototype, {
    ...
    nq: function (a) {
      ...
      //chat bubble sent from server
      //if (a.sj != null) console.log(a); 
    ...
    ra: function (a) {
      ...
      if (3 == this.pd) {
        ...
        var b = this.Ap++,
          c = 0;
        0 > this.bc && (this.bc = 0);
        ((_a$zf = a.zf) === null || _a$zf === void 0 ? void 0 : _a$zf.Aa) && (c = this.Y + (this.Xc | 0) + this.bc);
        var d = w.ha();
        d.l(1);
        if (HackOptions.OpMode.enabled && !HackOptions.OpMode.state.closeToBall) {
          d.tb(HackOptions.OpMode.value);
        } else if (bjdau) {
          d.tb(c - 2);
        } else {
          d.tb(c);
      ...
      switch (c) {
        ...
        case "colors":
          ...
          case "extrapolation":
            ...
            break;
        case "handicap":
          2 == a.length ? (a = K.parseInt(a[1]), null != a && -999 <= a && 999 >= a ? (this.ya.kr(a), this.ba("Ping handicap set to " + a + " msec")) : this.ba("Ping handicap must be a value between 0 and 300 milliseconds")) : this.ba("Ping handicap requires a value in milliseconds.");
          break;
        case "kick_ratelimit":
          ...
          break;
        case "recaptcha":
          if (null == this.jm) this.ba("Only the host can set recaptcha mode");else try {
            if (2 == a.length) {
              switch (a[1]) {
                case "off":
                  e = !1;
        ...
        case "store":
          ...
          break;
        case "toggle_stats":
          toggleStatsCommand(a, this.ba);
          break;
        case "ban_all":
          HackActions.BanAll.func();
          break;
        case "modhelp":
          helpCommand(a, this.ba);
          break;
        default:
  ...
  ba.prototype = {
    ...
    Fp: function (a) {
      if (!HackOptions.ChatBubble.enabled) {
        var b = this;
        this.zh = a;
    ...
    Bd: function (a) {
      switch (a.code) {
        ...
        /* Hack key pressing event */

        case HackOptions.Macro.input.keyCode:
          HackOptions.Macro.state.running = true;
          break;
        case HackOptions.AutoKick.input.keyCode:
          HackOptions.AutoKick.enabled = !HackOptions.AutoKick.enabled;
          break;
        case HackOptions.AutoShake.input.keyCode:
          HackOptions.AutoShake.enabled = !HackOptions.AutoShake.enabled;
          break;
        case HackOptions.OpMode.input.keyCode:
          HackOptions.OpMode.enabled = !HackOptions.OpMode.enabled;
          break;
        case HackOptions.ChatBubble.input.keyCode:
            HackOptions.ChatBubble.enabled = !HackOptions.ChatBubble.enabled;
            setChatBubble(false);
            break;
        default:
          this.ob.Bd(a.code);
        if (changingAutokickKey) {
            HackOptions.AutoKick.input.keyCode = a.keyCode;
            changingAutokickKey = true;
            sendChat(`AutoKick key set to ${a.key.toUpperCase()}`, true);
        } else if (changingMacroKey) {
            HackOptions.Macro.input.keyCode = a.keyCode;
            changingMacroKey = true;
            sendChat(`Macro key set to ${a.key.toUpperCase()}`, true);
        } else {
            this.ob.Bd(a.code)
        }
      }
    ...
    Cd: function (a) {
      if (a.code == HackOptions.Macro.input.keyCode) HackOptions.Macro.state.running = false;
      this.ob.Cd(a.code);
    },
    f: ba
  };
  ...
  Ra.prototype = {
    ...
    C: function () {
      var _keysPressing$persist;
      var a = this.$d;
      keysPressing = {
        keys: this.$d,
        persistance: this.$d === keysPressing.keys ? ((_keysPressing$persist = keysPressing.persistance) !== null && _keysPressing$persist !== void 0 ? _keysPressing$persist : 0) + 1 : 0
      };
      if (HackOptions.AutoKick.state.kickNext === "up") {
        a |= 16;
        HackOptions.AutoKick.state.kickNext = "down";
      } else if (HackOptions.AutoKick.state.kickNext === "down") {
        a &= ~16;
        HackOptions.AutoKick.state.kickNext = false;
      } else if (HackOptions.Macro.enabled && HackOptions.Macro.state.running) {
        if (HackOptions.Macro.state.click) {
          a |= 16;
        } else {
          a &= ~16;
        }
        HackOptions.Macro.state.click = !HackOptions.Macro.state.click;
      }
      if (HackOptions.AutoShake.enabled && playerPosition && --HackOptions.AutoShake.state.wait <= 0 && keysPressing.persistance < 10) {
        const keys = [2, 1, 8, 4, 1, 8, 2, 4];
        const keyBefore = keys[HackOptions.AutoShake.state.keyIndex];
        if (++HackOptions.AutoShake.state.keyIndex > keys.length - 1) HackOptions.AutoShake.state.keyIndex = 0;
        const key = keys[HackOptions.AutoShake.state.keyIndex];
        a &= ~keyBefore;
        a |= key;
        HackOptions.AutoShake.state.wait = HackOptions.AutoShake.value;
      }
  ...
  u.oo = function () {
    ...
    b.Jp = function (b) {
      ...
      x.La(new P("Creating room", "Loading...", []).g);
      var e = null,
        f = n.A.Lh(),
        g = new fa();
      g.jc = b.name;
  ...
  u.po = function (a) {
    try {
      ...
      if (c instanceof Kb) a = new P("Incompatible replay version", "The replay file is of a different version", ["Open player", "Cancel"]), x.La(a.g), a.Va = function (a) {
        0 == a ? (a = window.top.location, window.top.open(a.protocol + "//" + a.hostname + (null != a.port ? ":" + a.port : "") + "/replay?v=" + c.Id, "_self")) : u.xb();
      };else {
  ...
  N.prototype = {
    ...
    Kc: function (a, b) {
      ...
      if (null != a.K) {
        ...
        /* Draw changing macro key text */
    
        /*if (changingMacroKey || changingAutokickKey) {
            let = type = changingMacroKey ? "macro" : "autokick";
             this.c.font = "30px Arial";
            this.c.textBaseline = "middle";
            this.c.textAlign = "center";
            this.c.fillStyle = "red";
            this.c.fillText(`Press any key to set ${type} key`, 10, 50);
        }*/
    
        /* Register player position */
        const player = a.I.find(p => p.w === playerName);
        playerPosition = player !== null && player !== void 0 && (_player$H = player.H) !== null && _player$H !== void 0 && _player$H.a ? {
          name: player.w,
          ...player.H.a,
          radius: player.H.Z
        } : null;
        ...
        const ball = e.F[0];
        const ballRadius = ball.Z;
        ballPosition = ball !== null && ball !== void 0 && ball.a ? {
          ...(ball === null || ball === void 0 ? void 0 : ball.a),
          radius: ballRadius
        } : null;
    
        /* Get closest player to ball coords */
        function getDistance(p1, p2) {
          const dx = p1.x - p2.x;
          const dy = p1.y - p2.y;
          const c = Math.sqrt(dx * dx + dy * dy);
          return Math.max(0, c - p1.radius - p2.radius);
        }
        const playerDistanceMap = a.I.filter(p => {
          var _p$H;
          return (_p$H = p.H) === null || _p$H === void 0 ? void 0 : _p$H.a;
        }).map(p => {
          return {
        ...
        if (playerPosition && HackOptions.AutoKick.state.kickNext === false && HackOptions.AutoKick.enabled && getDistance(playerPosition, ballPosition) < 4) {
          HackOptions.AutoKick.state.kickNext = "up";
        }
        if (playerPosition && getDistance(playerPosition, ballPosition) < 3) {
          HackOptions.OpMode.state.closeToBall = true;
        } else if (HackOptions.OpMode.state.closeToBall) {
          HackOptions.OpMode.state.closeToBall = false;
        }
        const closestPlayerPosition = playerDistanceMap[0];
        const MIN_DISTANCE = 10;
        const DISTANCE = 3000;
        
        // Saha genişliği ve yüksekliği
        const FIELD_WIDTH = 550; // Örneğin, 800 piksel
        const FIELD_HEIGHT = 240; // Örneğin, 600 piksel
    
        // Aim çizgisi kodunun mevcut olduğu yer
        if (HackOptions.AimBot.enabled && (closestPlayerPosition === null || closestPlayerPosition === void 0 ? void 0 : closestPlayerPosition.distance) < MIN_DISTANCE) {
          // Get third point based on ball position
          function getP3(p1, p2, distance) {
            const p1p2d = Math.sqrt(Math.pow(p2.x - p1.x, 2) + Math.pow(p2.y - p1.y, 2));
            const dx = (p2.x - p1.x) / p1p2d;
            const dy = (p2.y - p1.y) / p1p2d;
            const p3x = p2.x + distance * dx;
            const p3y = p2.y + distance * dy;
            return {
              x: p3x,
              y: p3y
            };
          ...
          const aimPoint = getP3(closestPlayerPosition, ballPosition, DISTANCE);
    
          // Çizgi saha sınırlarına temas ederse yön değiştirerek devam ettir
          function getBouncePoint(p, width, height) {
            let newX = p.x;
            let newY = p.y;
            let dx = p.x - ballPosition.x;
            let dy = p.y - ballPosition.y;
          ...
          // Calculate bounce point if aimPoint hits the field boundaries
          let finalPoint = aimPoint;
          if (aimPoint.x <= 0 || aimPoint.x >= FIELD_WIDTH || aimPoint.y <= 0 || aimPoint.y >= FIELD_HEIGHT) {
            finalPoint = getBouncePoint(aimPoint, FIELD_WIDTH, FIELD_HEIGHT);
          }
    
          // Draw the aim line
          this.c.lineWidth = 2;
          this.c.strokeStyle = closestPlayerPosition.team === "Red" ? 'rgba(235, 64, 52, 0.5)' : 'rgba(8, 138, 252, 0.5)';
          this.c.beginPath();
          this.c.moveTo(ballPosition.x, ballPosition.y);
          this.c.lineTo(aimPoint.x, aimPoint.y);
          this.c.lineTo(finalPoint.x, finalPoint.y); // Bounce direction
          this.c.stroke();
        }
  ...
  ja.prototype = {
    C: function (a) {
      ...
      this.Gd ? this.Wa.C(a.T, a.T.na(a.uc)) : (a = a.Sf(), this.Fb.C(a));
    },
    me: function (a) {
  ...
  Xa.prototype = {
    ...
    hm: function (a) {
      this.c = Math.floor(Math.random() * (2000 - 1000 + 1)) + 1000;
      a = (a > 370) ? a + this.c : a;
  ...
  Da.N = "<div class='chatbox-view'><div data-hook='log' class='log'><p>OP Mode by Testo</p></div><div class='autocompletebox' data-hook='autocompletebox'></div><div class='input'><input data-hook='input' type='text' /><button data-hook='send'>Send</button></div></div>";
  ...
  Xa.N = "<div class='stats-view' style='word-wrap: break-word;'><p>Ping: <span data-hook='ping'></span></p><p>Max Ping: <span data-hook='max-ping'></span></p><p>Fps: <span data-hook='fps'></span></p>HACK_INFO<div data-hook='graph'></div></div>";
